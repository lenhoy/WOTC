...
def plot_trajectory(file_path=None):
    ...
    # State for legend
    state_vals = {'legend_loc': 'upper right'}
    ...
    if p0_available:
        ...
        # Initialize Markers (empty for now)
    ...
    def update_plot(val=None):
        ...
        # 3. Regenerate Current Quiver if params changed (or just always for simplicity in this callback)
        # Check if we need to regenerate quiver (grid or scale changed)
        # For simplicity, we regenerate it if it exists
        ...
        if visibility['current'] and vc_vals is not None:
            ...
            # If scale reduces (arrows get longer), width should increase
        ...
        if handles:
            try:
                ax.legend(handles, labels, loc=state_vals['legend_loc'], fontsize=FONT_SIZE)
            except Exception:
    ...
    fig_ctrl.text(0.8, 0.52, "Legend Loc", ha='center', fontsize=10, weight='bold')
    ax_rb = fig_ctrl.add_axes([0.65, 0.25, 0.3, 0.25])
    # Remove frame for cleaner look
    ax_rb.set_axis_off()
    
    leg_options = ['upper right', 'upper left', 'lower right', 'lower left', 'best', 'right']
    rb = RadioButtons(ax_rb, leg_options, active=0)
    
    def on_legend_change(label):
        state_vals['legend_loc'] = label
        update_plot()
    rb.on_clicked(on_legend_change)
